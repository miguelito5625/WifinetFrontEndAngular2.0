...
export class ListarClientesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  editarCliente(cliente) {
    ...
    this.router.navigate(['/administrador/clientes/modificar']);
  }

  agregarDireccion(cliente) {
    this.servicioClientes.clienteSeleccionado = cliente;
    this.router.navigate(['/administrador/clientes/agregar-direccion']);
  }

  listarDirecciones(cliente) {
    this.servicioClientes.clienteSeleccionado = cliente;
    this.router.navigate(['/administrador/clientes/listar-direcciones']);
  }

  detalleCliente(cliente) {
    this.servicioClientes.clienteSeleccionado = cliente;
    this.router.navigate(['/administrador/clientes/detalle']);
  }
  ...
  cambiarEstado(cliente) {
    ...
    if (cliente.total_instalaciones > 0) {
      Swal.fire({
        title: 'Error!',
        text: 'No puede cambiar de estado este cliente porque tiene instalaciones activas',
        icon: 'error',
        confirmButtonText: 'Aceptar'
      });
      return;
    ...
    const datos: any = {
      ...
      estado_cliente: cliente.estado === 'Activo' ? 'Inactivo' : 'Activo'
    }

    console.log(datos);


    this.servicioClientes.cambiarEstado(datos).subscribe(
      async (res) => {
        const result: any = res;
