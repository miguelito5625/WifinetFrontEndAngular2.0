...
export class ListarBoletasDepositoComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    $('#selectFiltroEstado').selectpicker('val', 'sin cuadrar');
  ...
  modificarBoleta(boleta) {
    ...
    this.router.navigate(['/administrador/boletas-deposito/modificar']);
  }

  gestionarTransacciones(boleta) {
    this.servicioBoletasDeposito.boletaDepositoSeleccionada = boleta;
    this.router.navigate(['/administrador/boletas-deposito/gestionar-transacciones-boleta']);
  }

  detalleBoleta(boleta) {
    this.servicioBoletasDeposito.boletaDepositoSeleccionada = boleta;
    this.router.navigate(['/administrador/boletas-deposito/detalle']);
  }
  ...
  cargarBoletasDeposito() {
    ...
    const subcribeBoletasDeposito = this.servicioBoletasDeposito.obtenerBoletasDeposito().subscribe(
      res => {
        const result: any = res;
  ...
  filtrarDatos() {
    ...
    // console.log(`filtro estado: ${val2}`);
