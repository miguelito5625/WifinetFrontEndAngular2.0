...
export class ListarCobrosComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  detalleCobro(cobro) {
    ...
    this.router.navigate(['/tecnicocobrador/cobros/detalles']);
  }
  ...
  cargarCobros() {
    ...
    const subcribeObtenerCobros = this.servicioCobros.obtenerCobrosFiltradosPorUsuarioCreador(id_usuario).subscribe(
      res => {
