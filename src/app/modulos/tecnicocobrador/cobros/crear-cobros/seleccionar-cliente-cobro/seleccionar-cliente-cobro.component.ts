...
export class SeleccionarClienteCobroComponent implements OnInit, OnDestroy {
  ...
  listarDirecciones(cliente){
   ...
   this.router.navigate(['/tecnicocobrador/cobros/crear/seleccionar-instalacion-cobro']);    
  }
  ...
  updateFilter(event) {
    ...
    const val1 = event.value.toLocaleLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").split(' ');
